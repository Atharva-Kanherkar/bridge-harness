...
| `UsageWidget`, `ContextBreakdown`, `HealthWarnings`, `BypassBadge` | Shared popover material, readable provider/history/login/context states, explicit unavailable measurements, compact actions. The composer popover measures its available space and ancestor clipping boundaries so its header stays below the toolbar when the window resizes. Health warnings retain shared alert semantics and persistent dismissal. Usage/context geometry remains computed at runtime. |
