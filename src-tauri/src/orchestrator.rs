...
An implementation result can open a durable completion gate. When routing metadata includes a completion state of `verifying` or `changes_requested`, continue sequentially: request the next required `verification` worker, include the implementation evidence ID, and wait for its structured result before claiming completion. Bridge—not prose—selects a verifier from a different harness family and rejects same-family passing evidence. Deterministic checks should run continuously; reserve model scrutiny and user-journey verification for meaningful completion boundaries. A `waived` result is human-approved risk, never equivalent to `verified`.
