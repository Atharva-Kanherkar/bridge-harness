...
pub fn open(path: &Path) -> Result<Connection, BridgeError> {
    ...
    let _ = connection.execute("ALTER TABLE sessions ADD COLUMN effort TEXT", []);
    let _ = connection.execute("ALTER TABLE sessions ADD COLUMN parent_session_id TEXT", []);
    let _ = connection.execute("ALTER TABLE sessions ADD COLUMN depth INTEGER", []);
    connection.execute(
