use crate::{
    adapters, completion,
    delegation::{
        SuggestedNextAction, WorkerEvidence, WorkerResult, WorkerResultStatus,
...
impl SessionSupervisor {
    pub fn track_adapter_process(
        ...
        let identity = adapters::process_identity(pid).ok_or_else(|| {
            BridgeError::Adapter(format!(
                "provider process {pid} has no verifiable OS identity"
            ))
        })?;
        ...
        if updated != 1 {
            return Err(BridgeError::Invalid(format!(
                "session {session_id} not found while tracking provider process"
            )));
        }
        Ok(identity)
    }
    ...
    pub fn recover_tracked_adapter_processes(db: &Connection) -> Result<usize, BridgeError> {
        ...
        for (session_id, status, raw_pid, expected_identity, was_mid_turn) in claims {
            ...
            let recovery_kind = if identity_matches {
                if !adapters::terminate_process_group(pid) {
                    return Err(BridgeError::Adapter(format!(
                        "tracked orphan process group {pid} could not be terminated"
                    )));
                }
            ...
            let active = !matches!(
                status.as_str(),
                "completed" | "cancelled" | "stopped" | "failed"
            );
    ...
    pub(crate) fn transition_in_transaction(
        ...
        let current: String = transaction
            ...
            .optional()?
            .ok_or_else(|| {
                BridgeError::Invalid(format!("worker runtime {session_id} not found"))
            })?;
    ...
    pub fn worker_evidence(
        ...
        if requested_ids.len() > MAX_EVIDENCE_REFERENCES {
            return Err(BridgeError::Invalid(format!(
                "worker evidence cannot contain more than {MAX_EVIDENCE_REFERENCES} references"
            )));
        }
        if requested_ids.iter().collect::<HashSet<_>>().len() != requested_ids.len() {
            return Err(BridgeError::Invalid(
                "worker evidence references cannot contain duplicates".into(),
            ));
        }
        ...
        let available = branch
            .into_iter()
            .filter(|entry| entry.kind == EntryKind::WorkerResult.as_str())
            .collect::<Vec<_>>();
            ...
            requested_ids
                ...
                .map(|id| {
                    available
                        ...
                        .ok_or_else(|| {
                            BridgeError::Invalid(format!(
                                "worker evidence {id} is not on the active parent branch"
                            ))
                        })
                })
                .collect::<Result<Vec<_>, _>>()?
        ...
        selected
            ...
            .map(|entry| {
                let child_session_id = entry
                    ...
                    .ok_or_else(|| {
                        BridgeError::Invalid(format!(
                            "worker evidence {} has no child session",
                            entry.id
                        ))
                    })?
                    .to_owned();
                ...
                let object = payload.as_object_mut().ok_or_else(|| {
                    BridgeError::Invalid(format!("worker evidence {} is not an object", entry.id))
                })?;
                ...
                let result: WorkerResult = serde_json::from_value(payload).map_err(|error| {
                    BridgeError::Invalid(format!(
                        "worker evidence {} is malformed: {error}",
                        entry.id
                    ))
                })?;
                result.validate().map_err(BridgeError::Invalid)?;
...
mod tests {
    ...
    fn restart_kills_matching_worker_process_then_reports_typed_recoverable_failure() {
        let db = database();
        db.execute(
            "UPDATE sessions SET status='working',active_turn_id='turn' WHERE id='child'",
            [],
        )
        .unwrap();
        db.execute(
            "UPDATE worker_runtime SET lifecycle_state='working' WHERE session_id='child'",
            [],
        )
        .unwrap();
        ...
        assert_eq!(
            SessionSupervisor::recover_tracked_adapter_processes(&db).unwrap(),
            1
        );
        let _ = child.wait();
        assert!(adapters::process_identity(pid).is_none());
        let tracked: (Option<i64>, Option<String>) = db
            .query_row(
                "SELECT adapter_pid,adapter_process_identity FROM sessions WHERE id='child'",
                [],
                |row| Ok((row.get(0)?, row.get(1)?)),
            )
            .unwrap();
        ...
        assert!(runtime.last_result.unwrap()["risks"][0]
            .as_str()
            .unwrap()
            .contains("partial"));
        ...
        assert!(entries
            .iter()
            .any(|entry| entry.payload["reason"] == "supervisor_restart_orphan"));
    ...
    fn restart_refuses_to_kill_pid_when_identity_does_not_match() {
        ...
        db.execute("UPDATE sessions SET status='stopped' WHERE id='child'", [])
            .unwrap();
        SessionSupervisor::reconcile_workspace_statuses(&db).unwrap();
        assert_eq!(
            db.query_row("SELECT status FROM workspaces WHERE id='w'", [], |row| {
                row.get::<_, String>(0)
            })
            .unwrap(),
            "failed"
    ...
    fn transition_persists_runtime_session_forest_and_audit_together() {
        ...
        let status: String = db
            .query_row("SELECT status FROM sessions WHERE id='child'", [], |row| {
                row.get(0)
            })
            .unwrap();
        ...
        let audit: i64 = db
            .query_row(
                "SELECT COUNT(*) FROM events WHERE entity_id='child' AND kind='worker.lifecycle'",
                [],
                |row| row.get(0),
            )
            .unwrap();
    ...
    fn illegal_transition_changes_nothing() {
        let db = database();
        assert!(
            SessionSupervisor::transition(&db, "child", WorkerLifecycleState::Completed, None,)
                .is_err()
        );
        assert_eq!(
            store::worker_runtime(&db, "child")
    ...
    fn typed_result_is_reported_and_releases_parent_exactly_once() {
        let db = database();
        db.execute("UPDATE sessions SET status='waiting' WHERE id='parent'", [])
            .unwrap();
        ...
        let reported = SessionSupervisor::record_result(&db, "child", &result)
            .unwrap()
            .unwrap();
        ...
        let (parent_status, lease_status): (String, String) = (
            ...
            .unwrap(),
            db.query_row(
                "SELECT lease_status FROM worker_leases WHERE session_id='child'",
                [],
                |row| row.get(0),
            )
            .unwrap(),
        ...
        assert_eq!(
            (parent_status.as_str(), lease_status.as_str()),
            ("ready", "released")
        );
    }
    ...
    fn evidence_selection_is_active_branch_only_and_malformed_records_fail_closed() {
        ...
        let result = WorkerResult {
            ...
            tests: vec![],
            decisions: vec!["Keep the API stable".into()],
            risks: vec![],
            remaining_work: vec![],
            suggested_next_action: SuggestedNextAction::Finish,
            suggested_role: None,
        ...
        let abandoned = forest
            .append("parent", EntryKind::WorkerResult, payload)
            .unwrap();
        let selected = SessionSupervisor::worker_evidence(
            &db,
            "parent",
            &[abandoned.id.clone(), reported.evidence_id.clone()],
        )
        .unwrap();
        assert_eq!(
            selected
                ...
                .collect::<Vec<_>>(),
            [abandoned.id.as_str(), reported.evidence_id.as_str()]
        );
        assert!(SessionSupervisor::worker_evidence(
            &db,
            "parent",
            &[reported.evidence_id.clone(), reported.evidence_id.clone()]
        )
        .is_err());
        forest
            .move_head("parent", Some(&reported.evidence_id))
            .unwrap();
        let non_result = forest
            .append(
                "parent",
        ...
        let malformed =
            serde_json::json!({"status":"completed","childSessionId":"child"}).to_string();
        db.execute(
            "UPDATE session_entries SET payload=?2 WHERE id=?1",
            params![reported.evidence_id, malformed],
        )
        .unwrap();
    ...
    fn default_evidence_is_deterministic_and_bounded_to_recent_results() {
        ...
        assert_eq!(
            evidence.last().unwrap().result.summary,
            format!("result-{MAX_EVIDENCE_REFERENCES}")
        );
    }
    ...
    fn restart_reconciles_working_waiting_and_warm_workers_without_orphans() {
        ...
        for (session_id, lifecycle) in [("waiting-child", "waiting"), ("warm-child", "warm")] {
            ...
            store::upsert_worker_runtime(
                ...
                &WorkerRuntimeRecord {
                    ...
                    task_family: "implementation".into(),
                    compatibility_key: format!("key-{session_id}"),
                    result_status: "pending".into(),
                    retry_count: 0,
                    warm_until: None,
                    worktree_path: None,
        ...
        assert_eq!(store::outstanding_children(&db, "parent").unwrap(), 0);
        let parent_status: String = db
            .query_row("SELECT status FROM sessions WHERE id='parent'", [], |row| {
                row.get(0)
            })
            .unwrap();
        ...
        let parent_results = store::session_entries(&db, "parent")
            .unwrap()
            .into_iter()
