...
pub fn search_github_repos(query: &str) -> Result<Vec<GithubRepoCandidate>, BridgeError> {
    ...
    let output = Command::new("gh").args(["api", "--paginate", "/user/repos?per_page=100&affiliation=owner,collaborator,organization_member"])
    ...
    let repos: Vec<GhRepo> = serde_json::from_slice(&output.stdout).map_err(|error| BridgeError::Adapter(format!("GitHub returned invalid repository data: {error}")))?;
