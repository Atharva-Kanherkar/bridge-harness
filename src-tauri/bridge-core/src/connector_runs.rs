...
///
/// `include_read_mentions` widens the window from unread to everything inside
/// it. It asks for no extra tool and no extra permission — the same read tools
/// answer both variants — and the ledger still announces each item exactly once,
/// so turning it on cannot replay an inbox. What it buys is a signal the user
/// can produce on demand: a mention already opened in the provider's own app
/// still arrives here once, which is the only way to tell a working pipe from a
/// quiet one without waiting for somebody else to send something.
pub fn ingress_prompt(family: ConnectorFamily, include_read_mentions: bool) -> String {
    ...
    // Off, these two are the wording ingress has always used, so the setting is
    // the only thing that can change a cycle's behaviour.
...
mod tests {
    ...
    fn the_ingress_prompt_reaches_read_items_only_when_asked() {
        // Why the pane could sit on "You're all caught up" forever: ingress asks
        // for the last hour *and not yet read*, so anyone who reads Slack in
        // Slack leaves it nothing to find. Correct for a notification surface,
        // and useless for answering "is this connector working".
