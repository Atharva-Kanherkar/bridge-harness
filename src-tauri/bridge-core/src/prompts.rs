...
pub const ORCHESTRATOR_SECTION_IDS: &[&str] = &[
    ...
    DELEGATION_PROTOCOL_SECTION_ID,
    ADDITIONAL_GUIDANCE_SECTION_ID,
];
pub const WORKER_SECTION_IDS: &[&str] =
    &[WORKER_CONTRACT_SECTION_ID, ADDITIONAL_GUIDANCE_SECTION_ID];
...
pub fn default_sections(target: PromptTarget, worker_depth: i64) -> Vec<PromptDefaultSection> {
    ...
    if target != PromptTarget::DirectSession {
        sections.push(PromptDefaultSection {
            id: ADDITIONAL_GUIDANCE_SECTION_ID,
            text: String::new(),
        });
    }
    sections
...
mod tests {
    ...
    fn target_inventories_match_the_live_prompt_shapes() {
            ...
            .filter(|target| matches!(target, PromptTarget::Worker(_)))
        {
            ...
            assert!(defaults[1].text.is_empty());
            assert!(!defaults[0].text.contains(RENDERING_NOTE));
        }
        assert_eq!(
            PromptTarget::DirectSession.section_ids(),
            [RENDERING_SECTION_ID]
        );
