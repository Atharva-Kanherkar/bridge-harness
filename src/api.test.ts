...
describe("the Work board", () => {
  it("shows an honest empty browser preview, without fabricated activity or local facts", async () => {
  ...
  it("hands back independent settings and stamps each cache read", async () => {
    const first = await bridgeApi.workBoard();
    first.settings.enabledConnectorInstances.push("invented");
    const second = await bridgeApi.workBoard();
    expect(second.settings.enabledConnectorInstances).not.toContain("invented");
    expect(Number.isFinite(Date.parse(second.generatedAt))).toBe(true);
  });
});
