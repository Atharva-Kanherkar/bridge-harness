...
describe("the api boundary consumes the generated contract", () => {
  ...
  it("routes every event subscription through the typed subscribe() helper", () => {
    ...
    expect(targets).toEqual(["notification", '"bridge-provider-usage-overviews"', '"bridge-menu-bar-settings-changed"', '"bridge-usage-overview"', '"bridge-menu-bar-settings"', '"bridge-meter-tray"', "MENU_COMMAND_EVENT", "AGENT_EVENT_BATCH"]);
  });
