...
export function ModelProfileEditor({ profiles, adapters, disabled, onChange }: {
  ...
  return <div className="space-y-3">
    {profiles.map(profile => <section key={profile.purpose} className="rounded-2xl border border-border bg-muted/50 p-3.5">
      <div className="mb-3 flex flex-wrap items-center justify-between gap-x-3 gap-y-1.5">
        ...
        <label className="flex items-center gap-2 text-[11px] text-muted-foreground"><input type="checkbox" checked={profile.learningEnabled} disabled={disabled || profile.pinned} onChange={event => update(profile.purpose, { learningEnabled: event.target.checked })} />Allow learning</label>
      </div>
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
        ...
        <label className="flex items-end pb-2 text-[11px] text-muted-foreground"><span className="flex items-center gap-2"><input type="checkbox" checked={profile.pinned} disabled={disabled} onChange={event => update(profile.purpose, { pinned: event.target.checked, learningEnabled: event.target.checked ? false : profile.learningEnabled })} />Pin this profile</span></label>
      </div>
    </section>)}
  </div>;
}
