// The mark a harness wears: in the startup row, the sidebar, the model control,
// and every Settings row that names a runtime. Inline SVG, no external asset
// fetches — the same rules `connectorLogos.tsx` follows, and for the same reason.
//
// These are the vendors' own marks, drawn from their published paths, not
// stand-ins. Earlier this file generated radially symmetric figures so that a
// slow rotation would scintillate; the rotation is gone (a brand mark that spins
// reads as a logo being spun), and with it the reason to invent shapes.
//
...
// open (see `harnessLabel`) — a mark is a nicety for the built-ins, not a
// requirement for the fifth.
//
// Color rules, from the design: Claude and OpenCode carry one tint each; OpenAI
// is drawn in the foreground ink and never in a green; Cursor keeps its own five
// facet greys, which invert between Graphite and Paper through the
// `--cursor-facet-*` tokens in `src/index.css`.

import { cn } from "@/lib/utils";

/** OpenAI's mark, worn by Codex. Foreground ink, never a tint. */
...
/** Anthropic's mark, worn by Claude Code. */
...
const CURSOR_FACETS: { points: string; facet: string }[] = [
  { points: "12.00 3.40 19.45 7.70 4.55 7.70", facet: "fill-[color:var(--cursor-facet-mid)]" },
  { points: "4.55 7.70 19.45 7.70 12 12", facet: "fill-[color:var(--cursor-facet-lightest)]" },
  { points: "4.55 7.70 12 12 12.00 20.60 4.55 16.30", facet: "fill-[color:var(--cursor-facet-light)]" },
  { points: "19.45 7.70 19.45 16.30 12.00 20.60", facet: "fill-[color:var(--cursor-facet-darkest)]" },
  { points: "12 12 19.45 7.70 12.00 20.60", facet: "fill-[color:var(--cursor-facet-lighter)]" },
];

/** Tint stays beside the figure so adding a harness cannot update only one. */
const TINTS: Record<string, string> = {
  claude: "text-harness-claude",
  opencode: "text-harness-opencode",
  // OpenAI and Cursor own their color: the first is drawn in the page's ink,
...
function figure(harness?: string | null) {
  switch (harness) {
    case "codex":
      return <path d={OPENAI_PATH} fill="currentColor" />;
    case "claude":
      return <path d={CLAUDE_PATH} fill="currentColor" />;
    case "opencode":
      return <>
    ...
    case "cursor":
      return <>
        ...
        <polygon
          points="12.00 3.40 19.45 7.70 19.45 16.30 12.00 20.60 4.55 16.30 4.55 7.70"
          fill="none"
          strokeWidth={0.6}
          strokeLinejoin="round"
          className="stroke-[color:var(--cursor-facet-light)]"
        />
      </>;
    default:
 ...
 * A harness's mark.
 *
 * `live` adds the breathe. Callers pass `live={false}` under
