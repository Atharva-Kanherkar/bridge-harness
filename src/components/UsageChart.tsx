...
import { HarnessMark, harnessChartDot, harnessChartStroke, harnessChartWash } from "./harnessMarks";
...
// Layered monotone areas, one per harness, each measured from zero. Each
// series wears its harness's chart colour — the same hue whether it is the
// only series or the fourth, so a filter never repaints the survivors — as a
// 2px line over a low-opacity wash. The chrome around them stays graphite.
const TICK_COUNT = 4;

/** Legend swatch for a harness series. */
export function seriesDotClass(harness: string): string {
  return harnessChartDot(harness);
}
    ...
    <div className="flex gap-2">
      ...
      <div ref={plotRef} className="relative h-56 min-w-0 flex-1" onMouseMove={onMove} onMouseLeave={() => setHover(null)}>
        <svg viewBox={`0 0 ${width} ${height}`} preserveAspectRatio="none" className="block h-full w-full overflow-visible" role="img" aria-label={`${resolution === "hour" ? "Hourly" : "Daily"} ${metric === "cost" ? "cost" : "processed tokens"} by harness`}>
          ...
          {/* All washes, then all lines, so no series covers another's line. The washes rise from the baseline once on mount. */}
          {paths.map((path, index) => <path key={`fill-${series[index].harness}`} d={path.area} className={cn(harnessChartWash(series[index].harness), "origin-bottom stroke-none motion-safe:animate-[chart-rise_600ms_ease-out_both]")} data-series={series[index].harness} />)}
          {paths.map((path, index) => <path key={`line-${series[index].harness}`} d={path.line} fill="none" className={harnessChartStroke(series[index].harness)} strokeWidth={2} strokeLinejoin="round" strokeLinecap="round" vectorEffect="non-scaling-stroke" />)}
        ...
        {hovered !== null && <div role="tooltip" className={cn("u-glass-popover pointer-events-none absolute top-2 z-10 min-w-40 rounded-xl px-3 py-2 text-caption", tooltipLeft > 60 ? "-translate-x-[calc(100%+12px)]" : "translate-x-3")} style={{ left: `${tooltipLeft}%` }}>
          ...
          {series.map(entry => <div key={entry.harness} className="flex items-center justify-between gap-4 text-muted-foreground">
            <span className="inline-flex items-center gap-1.5"><span className={cn("size-2 rounded-[3px]", seriesDotClass(entry.harness))} /><HarnessMark harness={entry.harness} size={11} />{harnessLabel(entry.harness)}</span>
