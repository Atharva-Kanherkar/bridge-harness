...
export function TasksPane({ sessions, runtimes = [], queue = [], terminalActivity, acknowledged, onAcknowledge, onOpenSession, onExpandWorker, onRetryWorker, onOpenTerminal }: {
  ...
  return <div className="flex h-full flex-col">
    <div className="min-h-0 flex-1 overflow-y-auto">
      {empty && <p className="px-3 py-6 text-center text-[12px] leading-relaxed text-muted-foreground">
        Nothing is running in the background for this session.
      </p>}
      ...
      {workerRows.map(({ runtime, session, status, broken }) => {
        ...
        return <div
          ...
          className={cn("group flex items-start gap-2.5 border-b border-border px-2.5 py-2.5 transition-colors hover:bg-accent", broken && "bg-destructive/5")}
        >
          ...
          >
            <span className="flex items-baseline gap-1.5">
              <b className="min-w-0 truncate text-[11.5px] font-medium text-foreground">{session.label}</b>
              <span className={cn("shrink-0 text-[9px] font-semibold tracking-[0.06em]", broken ? "text-destructive" : status.tone === "waiting" || status.tone === "attention" ? "text-warning" : "text-muted-foreground")}>{status.label}</span>
            </span>
            <small className={cn("mt-0.5 block truncate text-[10.5px]", broken ? "text-destructive" : "text-muted-foreground")}>
              {runtime.taskFamily}
              {elapsed ? ` · ${elapsed}` : ""}
          ...
          {broken && onRetryWorker && <button
            ...
            className="hidden h-6 w-6 shrink-0 place-items-center rounded text-muted-foreground hover:bg-card hover:text-foreground group-hover:grid"
          ...
          {onExpandWorker && <button
            ...
            className="mt-0.5 shrink-0 rounded text-muted-foreground/40 hover:text-foreground"
      ...
      {queue.map(item => <div key={item.id} data-task-row={item.id} className="flex items-start gap-2.5 border-b border-border px-2.5 py-2.5">
        ...
        <span className="min-w-0 flex-1">
          <span className="flex items-baseline gap-1.5">
            <b className="min-w-0 truncate text-[11.5px] font-medium text-foreground">{queueObjective(item.request) ?? "Queued delegation"}</b>
            <span className="shrink-0 text-[9px] font-semibold tracking-[0.06em] text-muted-foreground">{item.queueStatus.toUpperCase()}</span>
          </span>
          {/* An unexplained "pending" is the thing this pane exists to kill. */}
          <small className="mt-0.5 block truncate text-[10.5px] text-muted-foreground">
      ...
      >
        ...
        <span className="min-w-0 flex-1 truncate text-[11.5px] text-foreground">{shellCount} shell{shellCount === 1 ? "" : "s"} running</span>
        <ChevronRight size={12} className="shrink-0 text-muted-foreground/40" aria-hidden="true" />
      </button>}
    </div>
