...
// and drags the window from anywhere that is not a control. Windowed it sits
// flush on the canvas; fullscreen it spans the window with the panel on the
// left and history chevrons on the right.
...
export type AppTitleBarProps = {
  ...
  actions?: ReactNode;
  /** Panel toggle. In fullscreen it sits on the left, after the traffic lights. */
  leading?: ReactNode;
  /** History chevrons. In fullscreen they sit on the right with the other buttons. */
...
export function AppTitleBar({ title, navOpen, onOpenNav, actions, leading, trailingNav, flush = false, hideBrand = false }: AppTitleBarProps) {
  return (
    <header
