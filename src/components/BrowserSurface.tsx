  ...
  return <div className="relative flex h-full min-w-0 flex-col overflow-hidden bg-background">
    <header className="flex min-h-14 shrink-0 items-center gap-2 border-b border-border bg-muted/25 px-3 py-2">
      ...
      <div className="min-w-0 flex-1">
        <div className="truncate text-[11px] font-medium text-foreground">{snapshot?.tabs.find(tab => tab.attached)?.title ?? "Browser"}</div>
        <div className="truncate text-[11px] text-muted-foreground">{snapshot?.lease?.domain ?? "Use your existing logged-in tab"}</div>
      </div>
    ...
    {!snapshot ? <div className="grid flex-1 place-items-center text-muted-foreground"><LoaderCircle className="animate-spin" size={18} /></div> : !snapshot.nativeHostInstalled ? <SetupCard snapshot={snapshot} busy={busy} onInstall={() => void run(bridgeApi.installBrowserNativeHost)} /> : !snapshot.transportConnected ? <ConnectCard snapshot={snapshot} /> : !snapshot.lease ? <TabPicker snapshot={snapshot} busy={busy} onRefresh={() => void action("list_tabs")} onAttach={tabId => void action("attach", { tabId })} /> : <>
      <div className="flex flex-wrap items-center gap-1 border-b border-border bg-muted/20 px-2 py-1.5">
        {(["page", "elements", "timeline", "debug", "metrics"] as Pane[]).map(value => <button key={value} type="button" onClick={() => setPane(value)} aria-pressed={pane === value} className={`min-h-7 shrink-0 rounded-md px-2.5 py-1 text-[12px] capitalize ${pane === value ? "bg-selection text-selection-foreground" : "text-muted-foreground hover:bg-accent hover:text-foreground"}`}>{value}</button>)}
    ...
    {snapshot?.pendingApproval && <div className="absolute inset-0 z-20 grid place-items-center overflow-y-auto bg-background/80 p-5 backdrop-blur-sm">
      <div role="region" aria-label="Sensitive browser action" className="u-glass-popover w-full max-w-sm rounded-xl p-5">
...
function SetupCard({ snapshot, busy, onInstall }: { snapshot: BrowserBridgeSnapshot; busy: boolean; onInstall: () => void }) {
  return <div className="grid flex-1 place-items-center p-6"><div className="max-w-sm text-center"><Plug size={24} className="mx-auto text-muted-foreground" /><h2 className="mt-3 font-display text-base font-semibold text-foreground">Connect your browser once</h2><p className="mt-2 text-[13px] leading-relaxed text-muted-foreground">Register Bridge’s local native host, then load the Chrome extension. The grant is tab-specific; cookies and profile files never leave Chrome.</p><Button className="mt-4" size="sm" loading={busy} onClick={onInstall}>Register native host</Button><div className="mt-4 rounded-lg bg-muted p-2 text-left font-mono text-[11px] leading-4 text-muted-foreground"><div className="break-all">Extension ID: {snapshot.extensionId}</div><div className="break-all">Load unpacked: {snapshot.extensionPath}</div></div></div></div>;
}

function ConnectCard({ snapshot }: { snapshot: BrowserBridgeSnapshot }) {
  return <div className="grid flex-1 place-items-center p-6 text-center"><div className="min-w-0"><Chrome size={24} className="mx-auto text-muted-foreground" /><h2 className="mt-3 font-display text-base font-semibold text-foreground">Open the Bridge extension</h2><p className="mt-2 max-w-sm text-[11px] leading-5 text-muted-foreground">In Chrome, open <span className="font-mono text-foreground">chrome://extensions</span>, enable Developer mode, and load the unpacked extension shown below. Bridge reconnects automatically.</p><div className="mt-3 break-all rounded-lg bg-muted p-2 font-mono text-[11px] text-muted-foreground">{snapshot.extensionPath}</div></div></div>;
}
...
function TabPicker({ snapshot, busy, onRefresh, onAttach }: { snapshot: BrowserBridgeSnapshot; busy: boolean; onRefresh: () => void; onAttach: (id: number) => void }) {
  return <div className="flex min-h-0 flex-1 flex-col overflow-y-auto p-4"><div className="flex flex-wrap items-center gap-2"><div className="min-w-0"><h2 className="font-display text-sm font-semibold text-foreground">Attach one Chrome tab</h2><p className="mt-1 text-[11px] text-muted-foreground">Read-only is the default. Domain changes invalidate the lease.</p></div><Button variant="ghost" size="xs" className="ml-auto" loading={busy} onClick={onRefresh}><RefreshCw size={12} />Find tabs</Button></div><div className="mt-3 space-y-1.5">{snapshot.tabs.map(tab => <button type="button" key={tab.id} onClick={() => onAttach(tab.id)} className="u-glass-soft flex min-h-16 w-full items-center gap-3 rounded-xl p-3 text-left hover:bg-accent"><span className="grid size-7 shrink-0 place-items-center rounded-lg bg-muted"><Chrome size={13} /></span><span className="min-w-0 flex-1"><b className="block truncate text-[13px] font-medium text-foreground">{tab.title}</b><small className="block truncate text-[11px] text-muted-foreground">{tab.domain}</small></span><ChevronRight size={13} className="shrink-0 text-muted-foreground" /></button>)}{!snapshot.tabs.length && <div className="rounded-xl border border-dashed border-border p-6 text-center text-[11px] text-muted-foreground">Find tabs to show user-approved HTTP(S) pages.</div>}</div></div>;
}

function PageMirror({ snapshot, busy, onPoint }: { snapshot: BrowserBridgeSnapshot; busy: boolean; onPoint: (x: number, y: number) => void }) {
  if (!snapshot.screenshot) return <div className="grid min-h-full place-items-center p-6 text-center"><div><Camera size={24} className="mx-auto text-muted-foreground" /><p className="mt-2 max-w-sm text-[11px] leading-5 text-muted-foreground">{snapshot.captureError ? "Open the attached Chrome tab, click the Bridge extension icon, and choose Attach this tab to grant the live mirror." : snapshot.captureActive ? "Waiting for the first redacted mirror frame…" : "Attaching the live mirror…"}</p></div></div>;
...
function ElementsPane({ snapshot, selected, onSelect, text, onText, onActivate, onType }: { snapshot: BrowserBridgeSnapshot; selected?: BrowserElement; onSelect: (value: BrowserElement) => void; text: string; onText: (value: string) => void; onActivate: (value: BrowserElement) => void; onType: (value: BrowserElement) => void }) {
  return <div className="p-3"><div className="mb-2 flex items-center gap-1.5 text-[11px] text-muted-foreground"><Eye size={11} className="shrink-0" />Untrusted page content · {snapshot.elements.length} interactive elements</div><div className="space-y-1">{snapshot.elements.map(element => <button type="button" key={element.id} aria-pressed={selected?.id === element.id} onClick={() => onSelect(element)} onDoubleClick={() => onActivate(element)} className={`flex w-full items-start gap-2 rounded-lg border px-2 py-1.5 text-left ${selected?.id === element.id ? "border-border bg-accent" : "border-transparent hover:bg-accent"}`}><span className="mt-0.5 shrink-0 font-mono text-[11px] text-muted-foreground">{element.id}</span><span className="min-w-0 flex-1"><b className="block truncate text-[11px] font-medium text-foreground">{element.name || `[${element.role}]`}</b><small className="text-[11px] text-muted-foreground">{element.role}{element.sensitiveKind ? ` · ${element.sensitiveKind} (manual only)` : ""}{element.promptInjectionSuspected ? " · injection warning" : ""}</small></span></button>)}</div>{selected?.role === "textbox" && !selected.sensitiveKind && <div className="u-overlay sticky bottom-0 mt-2 flex gap-1.5 rounded-xl p-2"><input value={text} onChange={event => onText(event.target.value)} placeholder="Type into selected field" aria-label="Text for selected browser field" className="min-h-8 min-w-0 flex-1 rounded-lg border border-input bg-card px-2 text-[13px] outline-none focus:border-ring" /><Button size="xs" disabled={snapshot.lease?.permission !== "interact"} onClick={() => onType(selected)}>Type</Button></div>}</div>;
}

function TimelinePane({ snapshot }: { snapshot: BrowserBridgeSnapshot }) { return <div className="divide-y divide-border">{snapshot.audit.map(event => <div key={event.id} className="flex gap-2 px-3 py-2"><ScrollText size={11} className="mt-0.5 shrink-0 text-muted-foreground" /><div className="min-w-0"><b className="block text-[11px] font-medium text-foreground">{event.summary}</b><small className="text-[11px] text-muted-foreground">{event.kind} · {new Date(event.createdAt).toLocaleTimeString()}</small></div></div>)}</div>; }
function DebugPane({ snapshot, onEnable }: { snapshot: BrowserBridgeSnapshot; onEnable: () => void }) { return <div className="p-3"><Button variant="secondary" size="xs" onClick={onEnable}><TerminalSquare size={12} />Enable network & console inspection</Button><pre className="mt-2 whitespace-pre-wrap break-all rounded-xl bg-muted p-2 font-mono text-[11px] leading-4 text-muted-foreground">{snapshot.debugEvents.length ? snapshot.debugEvents.map(event => JSON.stringify(event)).join("\n") : "No debugger events captured."}</pre></div>; }
function MetricsPane({ snapshot, endpoint, tokenEnv, remoteUrl, onEndpoint, onTokenEnv, onRemoteUrl, onConfigure, onStart }: { snapshot: BrowserBridgeSnapshot; endpoint: string; tokenEnv: string; remoteUrl: string; onEndpoint: (value: string) => void; onTokenEnv: (value: string) => void; onRemoteUrl: (value: string) => void; onConfigure: () => void; onStart: () => void }) { return <div className="p-3"><div className="grid grid-cols-2 gap-1.5 sm:grid-cols-3">{[["DOM tokens", snapshot.tokenAccounting.estimatedInputTokens], ["Deltas", snapshot.tokenAccounting.deltaSnapshots], ["Screenshots", snapshot.tokenAccounting.screenshotCount]].map(([label, value]) => <div key={label} className="u-surface min-w-0 rounded-xl p-2"><div className="truncate text-[11px] uppercase tracking-wider text-muted-foreground">{label}</div><div className="mt-1 font-mono text-sm text-foreground">{value}</div></div>)}</div><div className="mt-2 space-y-1">{snapshot.siteMetrics.map(metric => <div key={metric.domain} className="rounded-lg bg-muted p-2 text-[11px] text-muted-foreground"><b className="block truncate text-foreground">{metric.domain}</b><div className="mt-1 break-words font-mono">{metric.successes}/{metric.actions} successful · {metric.actions ? Math.round(metric.totalLatencyMs / metric.actions) : 0}ms avg · {metric.inputTokens} tokens · {metric.interventions} takeovers · {metric.approvals} approvals · {metric.duplicateSideEffects} duplicate effects</div></div>)}</div><div className="mt-3 rounded-xl border border-border p-2"><div className="text-[11px] font-medium text-foreground">Optional remote browser</div><p className="mt-1 text-[11px] leading-4 text-muted-foreground">For geo/proxy, unattended, or high-concurrency tasks. Bridge stores only the HTTPS endpoint and environment-variable name—not the token.</p><div className="mt-2 grid gap-1.5"><input value={endpoint} onChange={event => onEndpoint(event.target.value)} aria-label="Remote browser endpoint" placeholder="https://browser.example.com" className="min-w-0 rounded-lg border border-input bg-background px-2 py-1.5 text-[11px] outline-none focus:border-ring" /><input value={tokenEnv} onChange={event => onTokenEnv(event.target.value)} aria-label="Remote browser token environment variable" placeholder="BRIDGE_REMOTE_BROWSER_TOKEN" className="min-w-0 rounded-lg border border-input bg-background px-2 py-1.5 font-mono text-[11px] outline-none focus:border-ring" /><Button variant="secondary" size="xs" onClick={onConfigure}>{snapshot.remoteProvider ? "Update provider" : "Save provider"}</Button>{snapshot.remoteProvider?.enabled && <div className="mt-1 flex gap-1"><input aria-label="Remote browser start URL" value={remoteUrl} onChange={event => onRemoteUrl(event.target.value)} className="min-w-0 flex-1 rounded-lg border border-input bg-background px-2 text-[11px] outline-none focus:border-ring" /><Button size="xs" onClick={onStart}>Start remote</Button></div>}</div></div></div>; }
