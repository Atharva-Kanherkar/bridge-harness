...
describe("AgentConversation", () => {
  ...
  it("shows launch failures and confirms that the orchestrator will not wait", () => {
    const html = renderToStaticMarkup(<AgentConversation session={session} onResolve={() => undefined} events={[
      event(1, "delegation.rejected", { role: "system", status: "failed", title: "Worker failed to start", text: "router schema mismatch", data: { phase: "routing", reason: "router schema mismatch", willRetry: false, orchestratorNotified: true } })
    ]}/>);
