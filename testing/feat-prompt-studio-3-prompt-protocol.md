...
- Worker contracts are depth-sensitive: every method accepts an optional `depth` (default 0, rejected when negative or above `DEFAULT_MAX_DEPTH`). Non-worker targets ignore it but echo the resolved value.
...
- Stack views report, per section: current state (`default` / `overridden {text}` / `deleted`), built-in default text, effective text (null when deleted), byte count and byte-derived token estimate of the effective text, lint warnings over the effective text, and the append-only revision history with operations and restore lineage.
...
- Preview returns, for one target:
  ...
  - `stack` (the full stack view, so revisions and lint travel with the preview);
  - `providerLayers` — one row per registered adapter's provider-base layer carrying a source status from the closed vocabulary `reported | measured | estimated | unavailable`, plus optional detail. No adapter can currently read back its provider base, so every row must be `unavailable` with the authority reason attached. Provider-owned text is never fabricated.
...
- `prompt_studio` module tests (bridge-core):
  - `stack_reports_states_defaults_sizes_lint_and_revisions` — default stack marks every section default; an override/delete flips state and effective text; sizes match effective-text bytes; token estimate is bytes.div_ceil(4); lint warnings appear on edited text that drops required markers; revision history is complete and ordered.
  ...
  - `provider_layers_cover_every_registered_adapter_with_honest_sources` — one row per entry in `provider_base_prompt_authorities()`, source vocabulary closed, every current row `unavailable` with a non-empty reason, and no row carries fabricated bytes.
- `protocol_mirror.rs` — `prompt_studio_payloads_mirror_core`: asserts `PromptStackView`, `PromptSectionView`, `PromptRevisionView`, `PromptSectionMutation`, `CompiledPromptPreview`, `PromptProviderLayerStatus`, and the section-state enum mirror their bridge-core counterparts document-for-document.
