...
1. **Switching model within the same harness** from the chat header: after the switch, the next provider turn receives the stored conversation as labelled restoration context. A question that depends only on earlier turns ("what file did we decide to change?") is answerable without re-explaining.
...
Explicitly out of scope (unchanged): same-harness native restarts (`RestorationPlan::Native` path), worker handoff, compaction triggers/policy.
