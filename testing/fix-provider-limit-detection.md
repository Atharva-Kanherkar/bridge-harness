...
- A reset stated as a wall-clock time carries no zone, so it is read on the user's clock and converted, never as UTC.
- The cooldown row and its `router.harness_quota_exhausted` event carry the harness, the signal, and the reset time.
- Refusing to compact is distinguishable from "a checkpoint is already pending". A warm worker whose expiry cannot be checkpointed still completes that expiry and retires, rather than staying warm and being reselected every maintenance tick.

Verification: replay real Codex, Claude and OpenCode limit frames through the live handler and assert a cooldown row with no `assistant.message` and no worker result present; assert reset-hint parsing including past hints and absent hints; assert the failed-lifecycle worker path writes it where slice 1's early return used to skip it; assert compaction refusal during cooldown and that an expired warm worker on an exhausted provider is retired exactly once. Run `bun run build` and `bun run test` before opening the PR.
