...
- Recursive scope inspection follows only links whose targets remain inside the canonical authorized subtree, detects nested escapes without cycles, and fails closed at a bounded traversal budget.
...
- A typed `Launched` outcome and spawned audit entry require successful objective delivery to the worker runtime; missing or rejecting runtimes settle the worker, release its lease, and produce the durable retryable failure path.
