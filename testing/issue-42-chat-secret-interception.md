...
9a. Every proxy call must present a per-launch session-private token (`x-bridge-proxy-auth`) delivered only through in-memory harness instructions. The persisted `[secret:sec_...]` reference alone is not a usable capability: a caller that scraped only the reference from history or the UI cannot replay a proxy call.
10. The loopback proxy supports bounded, non-streaming `GET`, `POST`, and `DELETE` requests, forwards only allowlisted request headers, returns response status/body without credential-bearing headers, and serves each call on its own thread so a slow upstream cannot block `/health` or serialize other agents.
