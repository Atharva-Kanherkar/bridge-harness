 ...
 * lit one is plain ink stencilled through the same 22px grid and a circle at the cursor,
 * so the dots near the pointer come up bright and the rest stay quiet.
 *
...
export default function HeroBackdrop() {
  ...
  return (
    ...
    >
      ...
      <div
        className="absolute size-[460px] -translate-x-1/2 -translate-y-1/2 rounded-full bg-foreground opacity-[calc(var(--glow)*0.07)] blur-3xl transition-opacity duration-700"
        style={{ left: "var(--mx)", top: "var(--my)" }}
      />

      {/* Ink showing through only the dots near the pointer. */}
      <div className="dot-spot absolute inset-0 bg-foreground opacity-[var(--glow)] transition-opacity duration-700" />
