...
- The mockup plays itself: `app/components/useDemoPlayback.ts` types the prompt, streams entries, reveals worker rows, and counts the diff up, then loops. Server-rendered HTML holds the finished state, so the page is complete without JavaScript, and `prefers-reduced-motion` skips playback entirely rather than hiding anything.
