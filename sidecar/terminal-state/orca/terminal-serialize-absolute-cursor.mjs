// Copyright (c) 2026 Lovecast Inc. MIT. Adapted from Orca f2d5711b2d32e9f11277cd63805c76b0b5f9ddf7; see THIRD_PARTY_NOTICES.md.
...
export function readSavedCursorRegister(terminal) {
    const core = terminal._core?.buffer;
    if (typeof core?.savedX !== 'number' ||
...
export function serializeWithAbsoluteCursor(serializer, terminal, opts, savedCursor) {
...
export function buildAbsoluteCursorRestoreSequence(terminal, savedCursor, options = {}) {
    const { cursorX, cursorY } = terminal.buffer.active;
    const terminalWithCore = terminal;
    ...
    const canRestoreCurrentCursor = cursorX >= 0 && cursorX < terminal.cols && cupCursorY >= 0 && cupCursorY < terminal.rows;
    ...
    const scrollRegionRestore = mustRestoreModes && (scrollTop !== 0 || scrollBottom !== terminal.rows - 1)
