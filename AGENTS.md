...
Node sidecar (`sidecar/claude-agent/`), not `claude -p`. Install its deps once
with `cd sidecar/claude-agent && npm install`, and keep `node` (18+) on PATH.
...
- **Fonts:** DM Sans (`font-sans`) for body, Bricolage Grotesque (`font-display`) for headings — loaded via `@fontsource*` in `main.tsx` and wired through `@theme` tokens.
