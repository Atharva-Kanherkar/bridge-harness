...
- **Agents:** the dock's Agents pane is the home for everything a conversation started — Bridge workers and harness-native subagents alike. It reads one pure projection, [`src/components/agentsModel.ts`](src/components/agentsModel.ts); the pane, the pinned tray and the chat's one-line pointer all read it and none keeps its own copy. Its contract is [`testing/feat-agents-pane.md`](testing/feat-agents-pane.md).
